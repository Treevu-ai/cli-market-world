"""
CLI Market Intelligence Client — Zoho integration.

Reutiliza los mismos endpoints canónicos que el prototipo HubSpot:
  Libres (solo auth):
    GET /v1/intel/brief
    GET /v1/intel/scores
...
PATH_PROCUREMENT_SIGNAL = "/v1/intel/procurement-signal"  # Pro — con guion, no guion bajo
PATH_PRICE_RISK         = "/v1/intel/price-risk"          # Pro — con guion, no guion bajo
PATH_BASKET             = "/v1/basket/compare"            # /v1/optimize no existe
...
class CLIMarketIntelClient:
    ...
    async def get_procurement_signal(self, country: str = "PE", line: str = "supermercados") -> dict[str, Any]:
        """Requiere tier Pro. Devuelve {"error": "tier_insufficient"} si no alcanza."""
    ...
    async def optimize_basket(self, products: list[str], country: str = "PE") -> dict[str, Any]:
        """
        POST /v1/basket/compare — requiere Pro+ en prod.
        El endpoint /v1/optimize del doc de arquitectura NO existe.
        """
        payload = {
    ...
    async def get_market_summary(self, country: str = "PE") -> dict[str, Any]:
        """Brief + scores + inflation en paralelo (endpoints libres)."""
    ...
    async def health_check(self) -> bool:
        try:
            ...
            logger.error("CLI Market health check failed: %s", e)
            return False
