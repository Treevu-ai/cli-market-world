    ...
    >
      <div className="landing-container-wide text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-[#3f3f46] mb-6">
              {isES ? "Uso" : "Usage"}
            </p>
            <div className="grid grid-cols-3 gap-4">
              {usageMetrics.map((m, i) => (
                <div key={i} className="text-center">
                  <p className="text-3xl font-black font-mono tabular-nums text-[#7CFF5B]">{m.value}</p>
                  <p className="text-xs mt-1 text-[#A1A1AA]">{m.label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {coverageMetrics.map((m, i) => (
                <div key={i} className="text-center">
                  <p className="text-3xl font-black font-mono tabular-nums text-[#FAFAFA]">{m.value}</p>
                  <p className="text-xs mt-1 text-[#A1A1AA]">{m.label}</p>
                </div>
              ))}
            </div>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
          {platforms.map((p) => (
            <span
              key={p}
              className="text-xs font-semibold font-mono px-3 py-1.5 rounded-full bg-[#18181B] border border-[#27272A] text-[#A1A1AA]"
            >
              {p}
            </span>
          ))}
