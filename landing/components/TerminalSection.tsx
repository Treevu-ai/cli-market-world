...
const cells: { title: string; lines: CmdLine[] }[] = [
  { title: "Search", lines: [
    ...
    { text: "▸ 3 resultados en 1.2s", color: "text-[#a3a3a3]", delay: 1500 },
  ]},
  { title: "Compare", lines: [
    ...
    { text: "▸ Ahorro: S/ 0.70 por unidad", color: "text-[#a3a3a3]", delay: 1300 },
  ]},
  { title: "Cart", lines: [
    { text: "$ market basket leche:2 arroz:1", delay: 300 },
    { text: "▸ Canasta comparada en 3 tiendas", color: "text-[#a3a3a3]", delay: 600 },
...
function MiniTerminal({ title, lines }: { title: string; lines: CmdLine[] }) {
  ...
  return (
    <div ref={ref} className="bg-white border border-[#e5e5e5] rounded-lg p-4 min-h-[140px]">
      <div className="flex items-center gap-1.5 mb-3 pb-2 border-b border-[#fafafa]">
        <span className="w-2.5 h-2.5 rounded-full bg-[#ff5f56]" />
        <span className="w-2.5 h-2.5 rounded-full bg-[#ffbd2e]" />
        <span className="w-2.5 h-2.5 rounded-full bg-[#27c93f]" />
        <span className="text-[10px] text-[#a3a3a3] font-mono ml-2 uppercase tracking-wider">{title}</span>
      </div>
      <div className="font-mono text-[11px] leading-relaxed space-y-0.5">
        {lines.map((line, i) => (
          <motion.div
            ...
            className={line.color || "text-[#525252]"}
          >
            {line.text}
          </motion.div>
        ))}
  ...
  return (
    <section id="terminal" className="relative bg-white py-20 border-t border-[#e5e5e5]">
      <div className="max-w-[720px] mx-auto px-6 text-center">
        <p className="text-xs text-[#a3a3a3] font-mono uppercase tracking-[0.15em] mb-8">{_t("terminal_label")}</p>
        <h2 className="text-[24px] font-medium text-black mb-3 tracking-tight">{_t("terminal_title")}</h2>
        <p className="text-sm text-[#737373] max-w-md mx-auto mb-12">{_t("terminal_desc")}</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
          {cells.map((cell) => (
            <MiniTerminal key={cell.title} title={cell.title} lines={cell.lines} />
          ))}
        </div>
        <p className="mt-8 text-[10px] text-[#a3a3a3] font-mono uppercase tracking-[0.15em]">{_t("terminal_footer")}</p>
      </div>
    </section>
  );
}
