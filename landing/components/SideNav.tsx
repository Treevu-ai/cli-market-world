...
export default function SideNav() {
  ...
  return (
    <nav
      className="fixed left-0 top-0 z-40 h-screen w-12 hidden xl:flex flex-col justify-center bg-[var(--cm-background)]/80 backdrop-blur-sm"
    ...
    >
      <div className="relative flex flex-col gap-4 px-3">
                ...
                {[0, 1, 2, 3].map((i) => (
                  <span
                    ...
                    className={`block h-0.5 w-0.5 rounded-full transition-colors ${
                      i === storyAct ? "bg-[var(--cm-mint)] scale-150" : "bg-white/20"
                    }`}
                  />
                ))}
              </span>
        ...
        <button
          ...
          className="mt-4 text-xs font-mono text-[var(--cm-on-surface-variant)] hover:text-white transition-colors z-10"
        >
          {isES ? "EN" : "ES"}
        </button>
      </div>
