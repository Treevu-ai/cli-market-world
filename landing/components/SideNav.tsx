...
export default function SideNav() {
  ...
  return (
    <nav
      className="fixed left-0 top-0 z-40 h-screen w-12 hidden xl:flex flex-col justify-center bg-[var(--cm-background)]/90 backdrop-blur-sm border-r border-[var(--cm-hairline-soft)]"
      aria-label={isES ? "Navegación de secciones" : "Section navigation"}
    >
      <div className="relative flex flex-col gap-4 px-3">
        <div className="absolute left-[calc(0.75rem+2.5px)] top-4 bottom-4 w-px bg-[var(--cm-outline-variant)]" aria-hidden="true">
          <div
                ...
                {[0, 1, 2, 3].map((i) => (
                  <span
                    ...
                    className={`block h-0.5 w-0.5 rounded-full transition-colors ${
                      i === storyAct ? "bg-[var(--cm-mint)] scale-150" : "bg-[var(--cm-text-secondary)]"
                    }`}
                  />
                ))}
              </span>
            ...
            <span
              className={`h-1.5 w-1.5 rounded-full transition-all duration-300 ${
                active === id
                  ...
                  : "bg-[var(--cm-text-secondary)] group-hover:bg-[var(--cm-on-surface-variant)]"
              }`}
            />
          </a>
        ))}
        <button
          ...
          className="mt-4 text-xs font-mono text-[var(--cm-on-surface-variant)] hover:text-[var(--cm-on-surface)] transition-colors z-10"
        >
          {isES ? "EN" : "ES"}
        </button>
      </div>
