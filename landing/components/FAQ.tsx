  ...
  return (
    <section id="faq" className="relative flex flex-col w-full bg-black py-16 px-6 lg:px-12 md:py-[80px] gap-8">
      <div className="flex flex-col gap-3 max-w-[600px]">
        <span className="inline-flex items-center gap-3 text-sm font-mono text-white/40"><span className="w-8 h-px bg-[#3cffd0]/40"/>FAQ</span>
        ...
        {faqKeys.map((f, i) => (
          <div key={i} className="bg-[#131313] border border-[#2d2d2d] overflow-hidden">
            <button onClick={() => setOpen(open === i ? -1 : i)} className="w-full flex justify-between items-center px-6 py-4 text-left">
              ...
              <span className="text-[#3cffd0] text-lg" style={{transform: open===i ? "rotate(45deg)" : "rotate(0)", transition: "transform 0.2s"}}>+</span>
            </button>
            {open === i && (
