...
const PANEL =
  "relative pointer-events-auto w-full sm:max-w-[960px] card-cyber flex flex-col max-h-[min(92dvh,720px)] overflow-hidden animate-fade-in";
...
export default function UseCaseDemoModal({
  ...
  return createPortal(
    <div className={OVERLAY} role="dialog" aria-modal="true" aria-labelledby="use-case-demo-title">
      ...
      <div className={PANEL}>
        <div className="shrink-0 flex items-start justify-between gap-4 p-4 sm:p-6 pb-3 border-b border-[var(--cm-outline-variant)]/20">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-4 sm:px-6 py-4">
          <div className="rounded-xl border border-[var(--cm-mint)]/35 bg-[#0a0a0a] overflow-hidden shadow-[0_0_40px_rgba(58,254,207,0.12)]">
            <img
              ...
              height={520}
              draggable={false}
              className="w-full h-auto max-h-[min(52dvh,480px)] object-contain object-top block mx-auto select-none"
              loading="eager"
              decoding="async"
            />
          </div>
        ...
        <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 p-4 sm:p-6 pt-3 border-t border-[var(--cm-outline-variant)]/20 bg-[var(--cm-surface-low)]">
          ...
          <div className="flex flex-wrap gap-2 ml-auto">
            <button
              type="button"
              onClick={onClose}
              className="text-xs font-mono text-[var(--cm-on-surface-variant)] hover:text-white px-4 py-2 border border-[var(--cm-outline-variant)]/40 rounded-lg"
            >
              {isES ? "Cerrar" : "Close"}
            </button>
