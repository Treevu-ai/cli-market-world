    ...
    >
      <div className="landing-container-wide flex items-center justify-between h-14 md:h-20 gap-4">
        <a href="/" className="flex items-center gap-2 shrink-0" aria-label="CLI Market home">
          <Logo />
          <span className="font-mono text-sm text-[var(--cm-on-surface)] font-semibold" style={{ letterSpacing: "-0.5px" }}>CLI Market</span>
        </a>
        ...
        <div className="hidden md:flex items-center gap-3 shrink-0">
          ...
          <a
            ...
            className="kimi-nav-link text-xs text-[var(--cm-text-secondary)] hover:text-[var(--cm-on-surface)] whitespace-nowrap transition-colors"
          >
            {signInCta}
          </a>
          <a
            href={CTA.signUp.href}
            className="inline-flex items-center rounded-[10px] bg-[var(--cm-mint)] text-[var(--cm-on-mint)] text-xs font-semibold px-4 py-2 hover:opacity-90 transition-opacity whitespace-nowrap shadow-sm"
          >
            {signUpCta}
          </a>
        </div>

        <button type="button" onClick={() => setOpen(!open)} className="md:hidden text-[var(--cm-text-secondary)] p-3" aria-expanded={open} aria-label={isES ? "Menú" : "Menu"}>
      ...
      {open && (
        <div className="landing-mobile-menu md:hidden border-t border-[var(--cm-outline-variant)] landing-container-wide py-4 flex flex-col gap-2 max-h-[calc(100dvh-3.5rem)] overflow-y-auto overscroll-contain safe-bottom bg-[var(--cm-surface-low)]">
          {TOP_NAV.map((item) => (
            <a
              key={item.id}
              href={item.href}
          ...
          <a href={CTA.signUp.href} onClick={close}
             className="inline-flex items-center justify-center rounded-[10px] bg-[var(--cm-mint)] text-[var(--cm-on-mint)] text-sm font-semibold px-6 py-3 mt-1 hover:opacity-90 transition-opacity">
            {signUpCta}
          </a>
          <button type="button" onClick={() => setLang(isES ? "en" : "es")}
            aria-label={isES ? "Switch to English" : "Cambiar a Español"}
            className="text-xs font-medium text-[var(--cm-text-secondary)] cursor-pointer text-left hover:text-[var(--cm-on-surface)] transition-colors">
            {isES ? "EN" : "ES"}
          </button>
        </div>
      )}
