  ...
  return (
    <section id="agents" className="relative flex flex-col w-full bg-black py-16 px-6 lg:px-12 md:py-[80px] gap-8">
      <div className="flex flex-col gap-3 max-w-[600px]">
        <span className="inline-flex items-center gap-3 text-sm font-mono text-white/40"><span className="w-8 h-px bg-[#00FF88]/40"/>{t("agent_label")}</span>
        ...
        {steps.map((s, i) => (
          <div key={i} className="bg-[#0A0A0A] border border-[#1A1A1A] overflow-hidden">
            <button onClick={() => setOpen(open === i ? -1 : i)} className="w-full flex items-center gap-4 px-6 py-4 text-left">
              <span className="text-[#00FF88] font-mono text-sm font-bold">{s.num}</span>
              <h3 className="font-grotesk text-base font-bold text-white">{t(s.titleKey)}</h3>
              <span className="ml-auto text-[#00FF88] text-lg" style={{transform: open===i ? "rotate(45deg)" : "rotate(0)", transition: "transform 0.2s"}}>+</span>
            </button>
            {open === i && (
