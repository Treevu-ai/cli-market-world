  ...
  return (
    <section id="how" className="relative bg-[#e8ebe6] py-20 border-t border-[#c5edab]">
      <div className="max-w-[720px] mx-auto px-6 text-center">
        <p className="text-xs text-[#454745] font-mono uppercase tracking-[0.15em] mb-8">{_t("how_label")}</p>
        ...
        <div ref={ref} className="bg-[#e8ebe6] border border-[#c5edab] rounded-lg overflow-hidden text-left">
          <div className="flex items-center gap-1.5 px-4 py-2.5 border-b border-[#fafafa]">
            ...
            <span className="text-[10px] text-[#454745] font-mono ml-2 uppercase tracking-wider">
              cli-market — bash
            </span>
          </div>
            ...
            {steps.map((s, i) => (
              <div key={i} className="group">
                <div className="flex items-start gap-2">
                  <span className="text-[#454745] select-none shrink-0 mt-[1px]">$</span>
                  <span className="text-[#454745]">{s.cmd}</span>
                </div>
                <div className="flex items-start gap-2 mt-0.5">
                  <span className="w-3 shrink-0" />
                  <span className="text-[#454745] text-[11px]">{isES ? s.out_es : s.out_en}</span>
                </div>
              </div>
            ))}
        ...
        <p className="mt-8 text-[10px] text-[#454745] font-mono uppercase tracking-[0.15em]">{_t("how_footer")}</p>
      </div>
    </section>
  );
}
