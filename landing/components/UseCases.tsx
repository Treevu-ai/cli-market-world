...
export default function UseCases() {
  ...
  return (
    <section id="usecases" className="wise-on-dark relative flex flex-col w-full bg-black py-16 px-6 lg:px-12 md:py-[80px] gap-8">
      <div className="flex flex-col gap-3 max-w-[600px]">
        <span className="inline-flex items-center gap-3 text-sm font-mono text-[var(--wise-mute)]"><span className="w-8 h-px bg-[#3cffd0]/40"/>{t("usecases_label")}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-[900px]">
        {cases.map((c) => (
          <div key={c.tKey} className="bg-[#131313] border border-[#2d2d2d] p-6 flex flex-col gap-4">
            ...
            <p className="text-[var(--wise-body)] text-sm font-sans leading-relaxed">{t(c.descKey)}</p>
          </div>
        ))}
      </div>
    </section>
