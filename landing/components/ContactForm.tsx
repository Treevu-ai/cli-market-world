...
export default function ContactForm({ initial = "pro" }: { initial?: string }) {
  ...
  if (sent) {
    return (
      <div className="bg-[#0A0A0A] border border-[#00FF88]/30 p-8 text-center flex flex-col items-center gap-4">
  ...
  return (
    <form onSubmit={submit} className="bg-[#0A0A0A] border border-[#1A1A1A] p-6 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {PLANS.map((p) => (
            <button key={p.key} type="button" onClick={() => setPlan(p.key)}
              className={`font-mono text-[11px] py-2 px-3 border text-center transition-all ${plan === p.key ? "border-[#00FF88] text-[#00FF88] bg-[#00FF88]/5" : "border-[#1A1A1A] text-[#555] hover:border-[#333]"}`}>
      ...
      <div>
        ...
        <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
          ...
          className="w-full bg-[#0F0F0F] border border-[#1A1A1A] px-4 py-2 font-mono text-sm text-white placeholder:text-[#444] focus:border-[#00FF88]/40 focus:outline-none" />
      </div>
      ...
      <div>
        ...
        <textarea required value={useCase} onChange={(e) => setUseCase(e.target.value)}
          ...
          rows={3} className="w-full bg-[#0F0F0F] border border-[#1A1A1A] px-4 py-2 font-mono text-sm text-white placeholder:text-[#444] focus:border-[#00FF88]/40 focus:outline-none resize-none" />
      </div>

      {error && <p className="font-mono text-[11px] text-[#FF6B35]">{error}</p>}

      <button type="submit" disabled={loading}
        className="bg-[#00FF88] text-black font-bold px-6 py-3 font-mono text-sm uppercase tracking-widest hover:bg-[#00cc6a] transition-colors disabled:opacity-50">
