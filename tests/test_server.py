...
def test_mcp_profile_defaults_to_default(monkeypatch):
    ...
    assert public_tool_count("default") == 24
...
def test_build_checkout_founding_uses_pro_founding_plan(monkeypatch):
    monkeypatch.setattr("server_deps.check_rate_limit", lambda _ip: None)

    async def fake_sub(username, **kwargs):
        assert kwargs.get("plan") == "pro_founding"
        return {
            "subscription_id": "I-FOUND99",
            "approve_url": "https://www.paypal.com/billing/subscriptions?ba_token=founding",
            "status": "APPROVAL_PENDING",
            "plan": "pro_founding",
            "amount": 29,
        }
    ...
    data = r.json()
    assert data["ok"] is True
    assert data["plan"] == "Pro Founding"
    assert data["amount"] == "$29/mo"
    assert data["approve_url"].startswith("https://")


def test_pro_checkout_paypal_founding_plan(monkeypatch):
    monkeypatch.setattr("server_deps.check_rate_limit", lambda _ip: None)

    async def fake_sub(username, **kwargs):
        assert kwargs.get("plan") == "pro_founding"
        return {
            "subscription_id": "I-FOUNDPC",
            "approve_url": "https://www.paypal.com/billing/subscriptions?ba_token=founding2",
            "status": "APPROVAL_PENDING",
        }
    ...
    r = client.post(
        ...
        json={
            "email": "founding2@cli-market.dev",
            "username": "founder2",
            "lang": "en",
            "payment_method": "paypal",
            "plan": "pro_founding",
    ...
    assert r.status_code == 200
    assert r.json()["plan"] == "Pro Founding"
