---
title: LinkedIn 30 días — Índice
tags:
  - linkedin
  - calendar
---

# LinkedIn 30 días — Índice

**Inicio campaña (reset):** 2026-05-29 = Día 1 → [[linkedin/catch-up-plan]]

**Fuente:** [[linkedin-calendar]] · **Estilo ES:** [[linkedin/STYLE-es]] · **Reglas:** 13:00 UTC · link en primer comentario · 60 min engagement

## Pilares

1. **Commerce × AI** — semana 1
2. **Data stories** — semana 2
3. **Build in public** — semana 3
4. **Social proof + CTA** — semana 4

## Semana 1 — Launch

| Día | Nota | Título | Estado |
|-----|------|--------|--------|
| 1 | [[Day-01]] | Agente comparó 30 retailers en 0.8s | `ready` |
| 2 | [[Day-02]] | Agentes no tienen dónde comprar | `ready` |
| 3 | [[Day-03]] | Stripe pagos / nosotros comercio | `ready` |
| 4 | [[Day-04]] | 5 agentes, leche Wong vs Metro | `ready` |
| 5 | [[Day-05]] | Carousel: 4 pasos agente compra | `ready` |
| 6 | [[Day-06]] | Por qué 22 herramientas MCP | `ready` |
| 7 | [[Day-07]] | [N] precios · 30 retailers (metrics) | `ready` ✅ [[metrics/price-pulse-2026-W22]] |

## Semana 2 — Data stories

| Día | Nota | Estado |
|-----|------|--------|
| 8 | [[Day-08]] | Arroz PE variación | `ready` ✅ [[linkedin/data-gate]] |
| 9 | [[Day-09]] | Canasta básica PE | `ready` ✅ cualitativo |
| 9b | [[Day-09-AR]] | Canasta AR trío | `draft` ⛔ gate AR |
| 10 | [[Day-10]] | Inflación collector 8h | `ready` ✅ |
| 11 | [[Day-11]] | Electro scale | `ready` ✅ |
| 12 | [[Day-12]] | Carousel top 10 | `ready` ⚠️ armar assets |
| 13 | [[Day-13]] | Retailers vs IA (EN) | `ready` |
| 14 | [[Day-14]] | 3 insights 19K+ | `ready` ✅ |

> ⚠️ Días 8–14: publicar solo con datos en [[linkedin/data-gate]] + [[metrics/price-pulse-YYYY-WW]]

## Semana 3 — Build in public

| Día | Nota | Estado |
|-----|------|--------|
| 15 | [[Day-15]] | De 1 VTEX a 30 retailers | `ready` |
| 16 | [[Day-16]] | 30 APIs sincronizadas | `ready` |
| 17 | [[Day-17]] | Python httpx asyncio | `ready` |
| 18 | [[Day-18]] | 36 MCP MIT | `ready` |
| 19 | [[Day-19]] | Collector Fly.io | `ready` |
| 20 | [[Day-20]] | Roadmap self-serve | `ready` |
| 21 | [[Day-21]] | VTEX inestables | `ready` |

## Semana 4 — CTA

| Día | Nota | Estado |
|-----|------|--------|
| 22 | [[Day-22]] | Case study retailer PE | `ready` |
| 23 | [[Day-23]] | VTEX/Magento listado | `ready` |
| 24 | [[Day-24]] | 3 razones listar | `ready` |
| 25 | [[Day-25]] | pip install CTA | `ready` |
| 26 | [[Day-26]] | Agent-first vision (EN) | `ready` |
| 27 | [[Day-27]] | 1000 agentes | `ready` |
| 28 | [[Day-28]] | Recap métricas | `ready` |
| 29 | [[Day-29]] | Gracias retailers | `ready` |
| 30 | [[Day-30]] | Milestone 30 días | `ready` |

[[GTM-Hub]]
