---
title: Day 20
status: ready
day: 20
pillar: build-in-public
lang: es
published_at:
link_comment: https://cli-market.dev
tags:
  - linkedin
---

# Day 20 — Roadmap self-serve PayPal

**Calendario:** [[linkedin-calendar]] · **Hub:** [[GTM-Hub]]

## Hooks (elegir 1)

1. **Hook 1:** ¿Qué sigue? Self-serve retailers. Pro live. Reportes semanales de precios.
2. **Hook 2:** Roadmap público: de Alpha a GA en 15 días.
3. **Hook 3:** Jun 1 Alpha · Jun 8 Beta · Jun 15 GA — esto viene.

## Post (copiar a LinkedIn — sin link en cuerpo)

¿Qué sigue en CLI Market?

**Ya shipped (Alpha):**
→ Billing Pro manual (email + PayPal)
→ Form self-serve en /retailers
→ 22 MCP tools + landing /tools
→ Collector + Price Pulse semanal

**Jun 8 Beta:**
→ HN launch + waitlist
→ 10 retailers self-serve
→ SEO agent-first live

**Jun 15 GA:**
→ Outbound escalado VTEX/Magento
→ Reportes Price Pulse PDF (Pro)
→ Path a $500 MRR

Transparencia total: collector hoy al 52% health — trabajando en llegar a 80%+.

Build in public = mostrar los números reales, no solo los buenos.

¿Qué feature priorizarías?

## Primer comentario

Roadmap + PRD 👇

https://cli-market.dev/retailers

## Hashtags

#buildinpublic #roadmap #startup #AI #ecommerce

## Assets

**Adjuntar en LinkedIn:** `docs/linkedin/assets/day-20/day-20-linkedin.png`
Regenerar: `python3 ops/generate_all_linkedin_assets.py --day 20` · todos: `python3 ops/generate_all_linkedin_assets.py`
## Checklist

- [ ] Mensaje alineado ([[GTM-Hub#Mensaje público acordado]])
- [ ] Datos verificados ([[linkedin/data-gate]] / [[metrics/price-pulse-YYYY-WW]]) si aplica
- [ ] Responder comentarios 60 min post-publicación

## Notas post-publicación

- Impresiones:
- Comentarios clave:
