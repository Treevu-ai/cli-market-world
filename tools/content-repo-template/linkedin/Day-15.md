---
title: Day 15
status: ready
day: 15
pillar: build-in-public
lang: es
published_at:
link_comment: https://cli-market.dev
tags:
  - linkedin
---

# Day 15 — De 1 conector VTEX a 30 retailers

**Calendario:** [[linkedin-calendar]] · **Hub:** [[GTM-Hub]]

## Hooks (elegir 1)

1. **Hook 1:** Arranqué CLI Market con un solo conector VTEX. Hoy son 30 retailers en 21 países.
2. **Hook 2:** Build in public: de un script Python a infraestructura de comercio para agentes.
3. **Hook 3:** El MVP era buscar leche en Wong. El producto es 22 herramientas MCP.

## Post (copiar a LinkedIn — sin link en cuerpo)

Arranqué CLI Market con un solo conector VTEX y una pregunta:

¿Puede un agente de IA comparar precios reales de supermercados en Perú?

Hoy:

→ 60 retailers (30 verificados)
→ 21 países (PE, AR, BR, MX, CO, CL, IT, FR)
→ 22 herramientas MCP
→ Collector automático cada 8 horas
→ Open source, MIT

El salto no fue "más código". Fue abstraer VTEX + Magento + auth + checkout detrás de una API.

Cada retailer que sumamos es un experimento: ¿responde la API? ¿Es estable? ¿Vale la pena?

Algunas tiendas fallaron. Las desactivamos. Las que funcionan, escalan.

Build in public no es solo marketing — es filtro de calidad.

¿Qué retailer te gustaría ver integrado?

## Primer comentario

GitHub 👇

https://github.com/Treevu-ai/cli-market-world

## Hashtags

#buildinpublic #AI #startup #ecommerce #opensource

## Assets

**Adjuntar en LinkedIn:** `docs/linkedin/assets/day-15/day-15-linkedin.png`
Regenerar: `python3 ops/generate_all_linkedin_assets.py --day 15` · todos: `python3 ops/generate_all_linkedin_assets.py`
## Checklist

- [ ] Mensaje alineado ([[GTM-Hub#Mensaje público acordado]])
- [ ] Datos verificados ([[linkedin/data-gate]] / [[metrics/price-pulse-YYYY-WW]]) si aplica
- [ ] Responder comentarios 60 min post-publicación

## Notas post-publicación

- Impresiones:
- Comentarios clave:
