...
## 📊 Datos reales (site_snapshot)

```json
{
  "url": "https://cli-market.dev/",
  "status_code": 200,
  "visible_text": "CLI Market — Infraestructura de comercio para agentes de IA Skip to main content CLI Market Build Procure Intelligence Retailers EN Iniciar sesión Obtener API Key INFRAESTRUCTURA DE COMERCIO PARA AGENTES DE IA Precios verificados. Hechos para que los use un agente. CLI, API y MCP sobre una sola capa de datos verificada. Elige cómo construyes, compras o analizas. CLI Build Integra en tu agente API, MCP y SDK sobre precios normalizados por kg/L. Free · Starter $9 · Pro $49/mes Obtener API Key → Procure Copilot Compra sin programar Comparar, aprobar y pagar en un solo flujo. Desde $29/mes · API incluida Ver Procure → Intelligence Señales antes del IPC Inflación, spreads y canasta desde góndola real. Piloto desde $300/mes Solicitar piloto → ¿Vendes en retail? Indexa tu catálogo gratis → Integra con Claude ChatGPT Cursor LangChain Vercel AI Hugging Face Mistral Gemini GitHub Copilot Claude ChatGPT Cursor LangChain Vercel AI Hugging Face Mistral Gemini GitHub Copilot CÓMO FUNCIONA De la góndola a tu producto Una capa de precios verificados y normalizados — consumida vía API, dashboard de compras o feeds de inteligencia. 1 Busca en todos los retailers a la vez Una consulta cruza tiendas verificadas en segundos — para un agente, un comprador o un analista. 2 Precios normalizados, listos para comparar Cada precio se convierte a la misma unidad — por kg, por litro, por unidad — para que la comparación sea justa y sin cálculos manuales. 3 Elige cómo consumirla Build: API y MCP. Procure: comparar y aprobar sin código. Intelligence: spreads e inflación. DEFENSABILIDAD El moat que no se copia. Grafo Retail descubiertos 82 activos y healthy 37 precios verificados 71,000+ Motor de Normalización normalizados kg/L/unidad 100% refresh 4h Toolkit para Agentes herramientas MCP 31+ MCP REST API CLI SDK Inteligencia Histórica indicadores 46 Inflation Volatility Price Risk Macro vs CPI CLI Market © 2026 CLI Market. Inteligencia de retail programable para LATAM. 82 retailers, 37 verificados activos · 71,000+ precios · MIT Producto CLI Build Procure Copilot Intelligence Cost of Living OS Mi hogar Desarrolladores Docs PyPI Compañía Casos de estudio Contacto Legal Acerca de Legal Términos de Servicio Política de Privacidad Licencia de Datos (ALD) Sinapsis Innovadora S.A.C. · RUC 20613045563 · Lima, Perú"
}
```
...
Producí el design system document + component library spec + layout framework descritos en tu contexto, evaluando la estructura real del sitio incluida abajo.
