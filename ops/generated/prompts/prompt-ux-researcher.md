...
```json
{
  ...
  "github": {
    "error": "Command '['gh', 'api', 'repos/Treevu-ai/cli-market-world', '--jq', '{stars: .stargazers_count, watchers: .subscribers_count, open_issues: .open_issues_count}']' returned non-zero exit status 1."
  }
}
```
...
Analizá el funnel (landing → pip install → market login → primera búsqueda → recurrente → Pro) usando las señales públicas reales de abajo (PyPI, GitHub) y evaluación heurística. Marcá toda suposición cualitativa como [HIPÓTESIS].
