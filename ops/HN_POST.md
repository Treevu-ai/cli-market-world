# Show HN: CLI Market — Commerce infrastructure for AI agents (38 retailers, 13 countries)

**Title:** Show HN: CLI Market — My agent shops at 38 verified retailers across 13 countries. One API.
...
CLI Market fixes this: one `pip install`, one API across 142 retailers (99 verified) in 13 countries. 22 curated MCP tools (46 legacy). ~50,000+ real prices refreshed every 4 hours. Checkout via PayPal (API) or QR (Yape/Plin).
...
The hard part isn't the code — it's maintaining connectors for 142 retailers whose APIs change without notice. We have VTEX, Shopify, and Magento connectors running a collector daemon every 4 hours. ~51,000+ prices indexed so far, growing to 200K+ as we roll out full catalog downloads.
