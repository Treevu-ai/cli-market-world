...
| `MARKET_BOT_API_TOKEN` | optional | world | Real Starter/Pro API token used by the WhatsApp/Telegram bridges for non-admin senders, so quota/tier limits apply per the bot account's own subscription instead of falling back to the admin token. **Pro required** for WhatsApp multi-line basket (`POST /v1/basket/compare` enforces `require_pro`). Product search works on lower tiers; intel path needs LLM provider credits (Anthropic in prod). Doc: `docs/integrations/whatsapp-bot.md`. |
