...
def db_set_subscription(username: str, tier: str, req_day: int | None = None,
                        req_min: int | None = None, expires_days: int | None = None,
                        paypal_subscription_id: str | None = None) -> dict:
    db = get_db()
...
def db_save_billing_pending(external_id: str, gateway: str, username: str,
                            kind: str = "subscription") -> None:
    db = get_db()
    if USE_PG:
        db.execute(
...
def db_create_subscription_request(username: str, email: str, payment_link: str) -> dict:
    ...
    db.commit()
    db.close()
    return {"id": req_id, "username": username, "email": email.strip().lower(), "payment_link": payment_link}
